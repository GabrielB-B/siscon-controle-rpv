...
class ConfigLocalTestCase(unittest.TestCase):
    ...
    def test_cookie_name_padrao_do_repositorio_atual_e_isolado(self):
        esperado = config_module._session_cookie_name_for_base_dir(config_module.BASE_DIR)
        self.assertEqual(config_module._default_session_cookie_name(), esperado)
        self.assertEqual(config_module.Config.REMEMBER_COOKIE_NAME, f"{esperado}_remember")

    def test_cookie_name_dev_e_isolado(self):
        self.assertEqual(
